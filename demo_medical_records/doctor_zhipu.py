...
"""
基于加权知识图谱的宠物问诊
(GLM-4.5 + 本地BGE + 中英桥接 + 概率推断 + 结构化1跳就诊建议/疾病总结)

实体类型适配：
- 用于推断：Disease, Symptom
- 仅用于内容（不参与推断）：Treatment, Pathogen, Host, Diagnosis, Prevention, DiseaseAlt(作为鉴别诊断)
...
class KGProb:
    ...
    从KG JSONL加载概率：
      ...
      - relation co_occurs_with: weight -> P(Si,Sj) 的联合（用于去冗余）
    仅 Disease/Symptom 进入推断。
    """
    ...
    def _load(self):
        ...
        for obj in load_jsonl(self.kg_path):
            ...
            if w is None: 
                continue
            w = float(w)
            # 只收 Disease->Symptom 的 presents_as 作为 P(S|D)
            if rel == "presents_as":
...
# ------- KG 一跳视图（用于就诊建议/疾病总结；含多类型节点但不参与推断） -------
class KGView:
    """
    加载完整 KG（节点+关系），提供：
      - disease_en_norm -> id （仅 Disease 作为诊断主体）
      - get_one_hop_subgraph(disease_en): 返回包含疾病节点、所有一跳邻居与边属性的结构体
    """
    ALLOWED_TYPES = {
        "Disease","Symptom","Treatment","Pathogen","Host","Diagnosis","Prevention","DiseaseAlt"
    }

    def __init__(self, kg_path: str):
        self.kg_path = kg_path
        self.nodes_by_id: Dict[str, dict] = {}
        self.out_edges: Dict[str, List[dict]] = defaultdict(list)  # head -> [relation_obj]
        self.in_edges: Dict[str, List[dict]]  = defaultdict(list)  # tail -> [relation_obj]
        self.dis_en2id: Dict[str, str] = {}
        self._load()

    def _load(self):
        for obj in load_jsonl(self.kg_path):
            if obj.get("type") in self.ALLOWED_TYPES:
                nid = obj.get("id")
...
# ---------------- 名称集合：用于过滤症状候选里的一切疾病风格词（含 DiseaseAlt） ----------------
def load_name_sets_by_type(kg_path: str, types: List[str]) -> Dict[str, Set[str]]:
    """
    返回: {type_name -> {norm_en(name), ...}}
    """
    ...
    for obj in load_jsonl(kg_path):
        ...
        if typ in out:
            en = (obj.get("text") or "").strip()
            if en:
                out[typ].add(norm_en(en))
    return out

# ---------------- Symptom Retriever (BGE + FAISS, 双语入库，仅Symptom) ----------------
class SymptomRetriever:
    """
    语料：英文症状 + （若有）中文译名，均指向同一英文canonical
    检索：中文短语 → topk 命中，按 canonical 聚合后取分数最高的若干个
    """
    def __init__(self, model_path: str, en_symptoms: List[str], en2zh_norm: Dict[str,str], topk: int = 2):
        ...
        for en in en_symptoms:
            self.corpus_texts.append(en)           # 英文
            self.corpus_canonical_en.append(en)
            zh = en2zh_norm.get(norm_en(en))
            if zh:
                self.corpus_texts.append(zh)       # 该英文的中文译名
                self.corpus_canonical_en.append(en)
...
# ---------------- Inference Engine (报告公式) ----------------
class Inference:
    """
    维护疾病后验 P(D|evidence)
    证据：S+（阳性症状英文）、S-（阴性症状英文）
    公式：
      初始：post[D] = P(D)
      阳性 s： 
        若尚无阳性：post[D] <- post[D] * P(s|D)/P(s)
        否则：post[D] <- post[D] * P(s|D)/max_j P(s|s_j)   (s_j ∈ 既往阳性)
      阴性 s：
        令 f = (1-P(s|D))/(1-P(s))，
        post[D] <- post[D] * f'
      步后截断 [0,1]；末尾归一化
    仅 Disease/Symptom 参与推断。
    """
    ...
    def build_candidate_from_topd(self, top_diseases: List[str], per_disease: int = 15) -> List[str]:
        """从Top疾病里取其 P(S|D) 最大的若干症状，并去掉已问过的"""
        cand = []
...
# --------- 生成“就诊建议”：用 1-hop 子图（不丢字段；显式标出鉴别诊断= DiseaseAlt） ---------
...
- 需把邻居按类别与关系名**分组罗列**：症状(Symptom, 基于 relation='presents_as' 及权重排序)、治疗(Treatment)、诊断检查(Diagnosis)、病原体(Pathogen)、宿主/易感(Host)、预防(Prevention)、鉴别诊断(DiseaseAlt)等。
- 可引用 knowledge_dump 中的 reference。
...
# --------- 生成“疾病总结”：用 1-hop 子图生成结构化中文总结（含 DiseaseAlt 作为鉴别） ---------
def generate_disease_summary(glm: GLM45, disease_en: str, disease_zh: Optional[str], subgraph: dict) -> str:
    disease_cn = disease_zh or disease_en
    dump_json = json.dumps(subgraph, ensure_ascii=False, indent=2)
    prompt = f"""
请基于下方疾病的一跳知识子图，产出**中文疾病总结**，覆盖：概述（中/英文名：{disease_cn} / {disease_en}）、典型症状(按 presents_as 权重排序)、病因/病原体、受累解剖/宿主因素、常用检查与诊断要点(Diagnosis)、治疗思路、预后、家庭护理与复诊建议、预防(Prevention)、鉴别诊断(DiseaseAlt)、红旗征象。若有 reference 请以“可参考资料”提及。  
...
def run_dialog(
    ...
    # 名称集合：用于把 Disease/DiseaseAlt 名字从“症状检索/候选”里剔除
    name_sets = load_name_sets_by_type(kg_path, ["Disease","DiseaseAlt"])
    disease_like_norm_set = set().union(*name_sets.values())

    # 症状全集（来自 P(S) 的键），并过滤掉任何落入“疾病风格集合”的名字
    all_symptoms_raw = list(kgp.symptom_prior.keys())
    filtered_symptom_list = [s for s in all_symptoms_raw if norm_en(s) not in disease_like_norm_set]
    ...
    # 4) 初始化推断
    engine = Inference(kgp, negative_alpha=0.5)
    ...
    while turn < max_turns:
        ...
        # 兜底检查：杜绝疾病/鉴别诊断名被拿来追问
        if nxt_en and norm_en(nxt_en) in disease_like_norm_set:
            nxt_en = None
