...
async def supervisor_tools(state: ReportState, config: RunnableConfig)  -> Command[Literal["supervisor", "research_team", "__end__"]]:
    ...
    if sections_list:
        ...
        next_section = sections_list[0]
        remaining_sections = sections_list[1:]
        return Command(
        goto="research_team",
        update={
            "messages": result,
            "section": next_section,
            "section_queue": remaining_sections
        }
        )
    elif intro_content:
...
async def supervisor_should_continue(state: ReportState) -> str:
    ...
    # 如果还有待处理 section，则继续分发
    if "section_queue" in state and state["section_queue"]:
        next_section = state["section_queue"][0]
        remaining = state["section_queue"][1:]
        return Command(
            goto="research_team",
            update={
                "section": next_section,
                "section_queue": remaining
            }
        )

    # 如果 LLM 没有工具调用，或是已结束，则退出
...
async def research_agent(state: SectionState, config: RunnableConfig):
    ...
    section_description = state.get("section", "General Research")
