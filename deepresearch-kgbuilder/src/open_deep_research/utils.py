...
async def tavily_search_async(
    ...
    """
    Performs concurrent web searches with the Tavily API (with concurrency & timeout control)
    """
    tavily_async_client = AsyncTavilyClient()

    # 控制同时并发的最大搜索数量
    semaphore = asyncio.Semaphore(2)  # 可根据网络情况调整 2～5

    async def safe_search(query):
        async with semaphore:
            try:
                print(f"[🌐] Tavily 搜索中: {query}")
                return await tavily_async_client.search(
                    query,
                    max_results=max_results,
                    include_raw_content=include_raw_content,
                    topic=topic,
                    timeout=90  # 每个请求最多等待 90 秒
                )
    ...
    # 并发执行搜索任务
    search_tasks = [safe_search(query) for query in search_queries]
